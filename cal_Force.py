...
for i in range(subnum):
    # 入力フォルダと出力先フォルダ名を指定
    input_folder = "D:/User/kanai/Data/240601/sub%d/csv/motion" % (i+1)
    ...
    for root, dirs, files in os.walk(input_folder):
        ...
        for filename in file_list:
            ...
            # 指先の風船重りに対する相対加速度を計算
            R = (diff.diff(-3, axis=1))/(delta_t**2)
            print(df.columns[[3, 4, 5]])
            R = R.drop(columns=df.columns[[3, 4, 5]], axis = 1)
            R = R.rename(columns={"hand X":"Force X", "hand Y":"Force Y", "hand Z":"Force Z"})
            
            # 力覚を算出
            F = R * m
            F.iloc[:, 2] = F.iloc[:, 2] + m*g
